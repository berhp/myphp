...
class Upload{
	 ...
	 * 【容器】-勿设置值
	 */
	private $_maxSize; 	//当前上传文件的大小,单位字节
    ...
    private function _action_maxSize(){
    	$this->_maxSize = $this->maxSize ? $this->maxSize*1024*1024 : 9999*1024*1024; 
    }
    ...
    private function _check_filetype($name=''){
    	...
    	if(in_array($filetype, $this->exts)){
    		$this->_filetype = $filetype;
    		return true;
    	}else{
    		$this->_msg .= '文件"'.$name.'"后缀名不支持'.$filetype;
    		return false;
    	}
    } 
